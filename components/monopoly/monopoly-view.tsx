...
import { Player, PlayerJSON } from "@/lib/monopoly-player";
import "@/lib/monopoly.css";
import MonopolyNav, { MonopolyNavRef } from "@/components/monopoly/ingame/nav";
import MonopolyGame, { MonopolyGameRef } from "@/components/monopoly/ingame/game";
import NotifyElement, { NotificatorRef } from "@/components/monopoly/notificator";
import monopolyJSON from "@/lib/monopoly.json";
import { MonopolySettings, MonopolyModes, historyAction, history, GameTrading, MonopolyMode, MonopolyModeSearch } from "@/lib/monopoly-types";
import { CookieManager } from "@/lib/monopoly-cookieManager";
import { SocketBroker } from "@/components/providers/monopoly-socket-provider";

export function MonopolyView({ broker, name }: { broker: SocketBroker, name: string }) {
    ...
    useEffect(() => {
        ...
        const socket_TurnFinished = (args: { from: string; turnId: string; pJson: PlayerJSON; WinningMode: string }) => {
            ...
            if (args.WinningMode === "monopols" || args.WinningMode === "monopols & trains") {
                const removeDuplicates = (originalList: Array<any>) => {
            ...
            setTimeout(() => {
                ...
                switch (c.action) {
                    ...
                    case "movenearest":
                        ...
                        const findNextValue = (arr: number[], X: number) => {
